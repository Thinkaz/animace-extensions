...
export class AnimaceParser {
    ...
    parseWeeklyRecommendations(
        html: string,
    ): PagedResults<DiscoverSectionItem> {
        const $ = cheerio.load(html);
        const items: DiscoverSectionItem[] = [];

        $(".carousel.home-novels-sld .col-6.col-sm-4.col-xl-2.p-3.post").each(
            (_i, el) => {
                const $el = $(el);
                ...
                const imageUrl = this.helper.fixImageUrl(
    ...
    parseChapterDetails(
        ...
        if (contentContainer.length === 0) {
            throw new Error(
                "Content container not found. Unable to extract images.",
            );
        }
        ...
        contentContainer.find("img").each((_index, el) => {
            ...
            if (
                this.excludePatterns.some((pattern) => imgUrl.includes(pattern))
            ) {
                return;
            }
